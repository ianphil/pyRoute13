...
class RoutePlanner:
            ...
            if node.key < len(actions):
                ...
                if leaf_node:
                    ...
                    if previous_state.working_time != 0:
                        score = (
                            previous_state.quantity_unloaded
                            / previous_state.working_time
                        )
    ...
    def _apply_action(
        ...
        def case_drop_off(
            ...
            if action.location != state.location:
                ...
                if logger:
                    travel_time = trace.format_time_HMS(transit_time)
                    msg = "    {}: drive for {} to location {}".format(
                        state.time, travel_time, action.location
                    )
                    logger(msg)
        ...
        def case_pickup(
            ...
            if action.location != state.location:
                ...
                if logger:
                    travel_time = trace.format_time_HMS(transit_time)
                    # travel_time = transit_time / 60 if transit_time >= 60 \
                    #     else transit_time
                    # time_format = 'min' if transit_time >= 60 else 'sec'
                    # msg = '    {}: drive for {:.1f}{} to location {}'.format(
                    #        state.time, travel_time, time_format,
                    #        action.location)
                    msg = "    {}: drive for {} to location {}".format(
                        state.time, travel_time, action.location
                    )
        ...
        try:
            action_cases[action.type](self, capacity, state, action, logger)
        except KeyError:
    ...
    def _actions_from_jobs(self, jobs: List) -> List:
        ...
        def case_out_of_service(self, jobs: List):
            for job in jobs:  # type: OutOfServiceJob
        ...
        def case_transfer(self, jobs: List):
            for job in jobs:  # type: TransferJob
        ...
        for job in jobs:
            try:
                job_cases[job.job_type](self, jobs)
            except KeyError:
                pass
